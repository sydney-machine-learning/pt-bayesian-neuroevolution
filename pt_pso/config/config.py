...
parser.add_argument('--problem', type=str, default="iris", help='Problem to be used for Evolutionary PT: \n"synthetic", "iris", "ions", "cancer", "bank", "penDigit", "chess", "TicTacToe')
parser.add_argument('--num-chains', type=int, default=10, help='Number of Chains for Parallel Tempering')
parser.add_argument('--population-size', type=int, default=50, help='Population size for G3PCX Evolutionary algorithm.')
parser.add_argument('--num-samples', type=int, default=50000, help='Total number of samples (all chains combined).')
parser.add_argument('--swap-interval', type=int, default=100, help='Number of samples between each swap.')
parser.add_argument('--burn-in', type=float, default=.50, help='Ratio of samples to be discarded as burn-in samples. Value 0.1 means 10 percent samples are discarded')
...
if opt.config_file is None:
    opt.config_file = os.path.join(opt.root, 'pt_pso', 'config', 'data.yaml')
